...
class AllCNN(nn.Module):
    def __init__(self, filters_percentage=1., n_channels=3, num_classes=10, dropout=False, batch_norm=True, dataset='cifar'):
...
class FLNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(F.relu(activation1), 2)
        activation2 = self.conv2(x)
        x = F.max_pool2d(F.relu(activation2), 2)
        x = x.view(-1, x.shape[1] * x.shape[2] * x.shape[3])
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x, activation1, activation2
...
class ClientResNet(nn.Module):
    def __init__(self):
        ...
        # Match your original modification
        full_model.conv1 = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
        
        # Client executes up to the first residual block
...
class ServerResNet(nn.Module):
    def __init__(self):
        ...
        # Server executes the rest of the residual blocks
        ...
        self.fc = nn.Linear(512, 10) # Match your original modification
...
class ClientAllCNN(nn.Module):
    def __init__(self, filters_percentage=1., n_channels=3, batch_norm=True):
        super(ClientAllCNN, self).__init__()
        n_filter1 = int(96 * filters_percentage)
        n_filter2 = int(192 * filters_percentage)

        # Client executes the first 3 convolutions
        self.client_layers = nn.Sequential(
            ...
            nn.Dropout(inplace=True) # dropout1
        )
...
class ServerAllCNN(nn.Module):
    def __init__(self, filters_percentage=1., num_classes=10, batch_norm=True, dataset='cifar'):
        super(ServerAllCNN, self).__init__()
        n_filter2 = int(192 * filters_percentage)

        # Server executes the remaining convolutions and classifier
        self.server_layers = nn.Sequential(
            ...
            nn.Dropout(inplace=True), # dropout2
            Conv(n_filter2, n_filter2, kernel_size=3, stride=1, batch_norm=batch_norm),
