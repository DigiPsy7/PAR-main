...
def divergence(student_logits, teacher_logits, KL_temperature):
    divergence = F.kl_div(F.log_softmax(student_logits / KL_temperature, dim=1),
                          F.softmax(teacher_logits / KL_temperature, dim=1))  # forward KL
...
def KT_loss_student(student_logits, student_activations, teacher_logits, teacher_activations, KL_temperature=1,
                    AT_beta=250):
    divergence_loss = divergence(student_logits, teacher_logits, KL_temperature)
    if AT_beta > 0:
        at_loss = 0
...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, dataset, num_channels=3):
        ...
        if dataset == 'mnist' or dataset == 'fashion-mnist':
            layers.extend([
                nn.Conv2d(64, 64, 5, stride=1, padding=0),  # Adjust from 32x32 to 28x28
                nn.BatchNorm2d(64),
                nn.LeakyReLU(0.2, inplace=True)
            ])
        
        layers.extend([prefinal_layer, final_layer])
...
class LearnableLoader(nn.Module):
    def __init__(self, n_repeat_batch, dataset, batch_size=128, num_channels=3, device='cuda'):
        """
        ...
        self.z_dim = 128
        self.generator = Generator(self.z_dim, dataset=dataset, num_channels=num_channels).to(device=device)
        self.device = device
